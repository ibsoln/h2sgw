...
create_target::
_Non-functional_.
The _create_target_ property remains in `ReplicationConfig` but is redundant for Couchbase Server-backed use cases.
// end::create-target[]

// tag::proxy[]
proxy::
_Non-functional_.
The _proxy_ property remains in `ReplicationConfig` but is non-functional.
// end::proxy[]

// tag::doc-ids[]
doc_ids::
_Non-functional_.
The _doc_ids_ property remains in `ReplicationConfig` but is non-functional.
// end::doc-ids[]
...
_Removed - redundant_.
+
The _source_ property is, implicitly, the database associated with the replication (that is, the 'database' property under which it is defined).
// end::source[]
...
_Removed - redundant_.
+
The _target_ property is replaced by the `<<remote-property>>` property.
// end::target[]
