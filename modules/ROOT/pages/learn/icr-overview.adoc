...
Couchbase Sync Gateway supports inter-cluster replication, the ability to run replications between two Sync Gateway clusters and between active mobile clusters.

This is an increasingly important feature and is essential to support _{glos-term-cloud-to-edge-sync}_ use cases. Here data changes must be synchronized between a centralized cloud cluster and a large number (hundreds or thousands) of edge clusters whilst still enforcing fine grained access control.
