...
It involves a hierarchy of couchbase mobile clusters, within which a large number of edge clusters sync with one or more clusters in cloud data centers.
// end::sgw-cloud-to-edge-sync[]
+
...
// end::sgw-cloud-to-edge-sync-def[]
+
* _Synonym(s)_: ship-to-shore sync, hub-and-spoke sync
* _SGW Component_: Inter-cluster replication
* _Related Term(s)_:
* _Read More_: {xref-sgw-pg-icr-overview}
// end::sgw-cloud-to-edge-sync-full[]
