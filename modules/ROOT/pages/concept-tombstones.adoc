...
A _tombstone_ is a document revision that contains only:
* the document ID
* a revision ID
* a key value pair `deleted:true`.

The tombstone acts as the record of a deleted item.
It is created to allow all devices to see that a document has been deleted - particularly in the case of devices that may not be online continuously and therefore not syncing regularly.
